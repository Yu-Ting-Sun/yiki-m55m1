...
static void SlideFilter_Update(const char *label)
{
    if (label != NULL)
    {
        ...
        {
            if (strncmp(s_candUser, s_filterUser, sizeof(s_filterUser)) != 0)
            {
                strcpy(s_filterUser, s_candUser);
            ...
            printf("[LOCK] holding '%s' for %u ms — recognition paused\n",
                   s_filterUser, (unsigned)LOCK_HOLD_MS);
        }
    }
    ...
    if (Display_Init() == 0)
    {
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                {
#if RUN_CAMERA_PREVIEW
                    /* Locked onto a viewer: face recognition pauses (the box
                     * stops flickering red/green) and the preview corner is
                     * wiped once, leaving a clean photo frame. Capture and the
                     * like gesture deliberately keep running — being locked is
                     * exactly when somebody is standing in front of the frame,
                     * ready to thumbs-up the photo. */
                    bool locked = false;
#if RUN_FACE_DETECT && RUN_FACE_RECOG && !RUN_FACE_ENROLL
                    locked = SlideFilter_IsLocked();
...
#endif
#if !RUN_GESTURE_LIKE
                    /* Without the like gesture nothing else needs the camera
                     * while locked, so idle cheaply (the PR's behaviour). */
                    if (locked)
                    {
                        Display_Delay(50);
                        continue;
...
#endif
                        if (fdOk && !locked && FaceDetect_Run(frame, CAM_W, CAM_H) > 0)
                        {
                            FaceBox tb;
